...
bot.command("nosho2", (ctx) => {
	...
	if (ctx.message.from.last_name) {
		restaurante  = ctx.message.from.first_name+ " "+ctx.message.from.last_name
	}
	else {
		restaurante = ctx.message.from.first_name
	}
	...
	const opts = {
		"reply_markup": {
            "inline_keyboard": [[
                {
                    "text": "Sim",
                    "callback_data": "nosho 0 " +sellerId + " "+restaurante+" Sim"            
                }, 
                {
                    "text": "Não",
                    "callback_data": "nosho 0 "+sellerId + " "+restaurante+" Não"            
                }
				]
            ]
        }
	...
	bot.telegram.sendMessage(sellerId, "Olá " +restaurante+ " ,deseja publicar um NoSho:", opts)
...
bot.on('message', (ctx) => {
	...
	if (ctx.message.contact) {
		...
		console.log(currentSell.sellerId)
		console.log(currentSell.buyerId)
	...
	if (replyToText == "Por favor insira o nome do seu restaurante:" ) {   
...
bot.action(/[0-9]/, (ctx) => {
	...
	if (command == "nosho") {
		...
		if (msg[1] == "0") {
			...
			let restaurant
			restaurant = calldata.splice(2, calldata.length-3)
			restaurant = restaurant.join(" ")
			...
			const opts = {
				"reply_markup": {
					"inline_keyboard": [[
						{
							"text": "2",
							"callback_data": "nosho 1 "+sellerId + " "+restaurant+" 2"            
						}, 
						{
							"text": "4",
							"callback_data": "nosho 1 "+sellerId + " "+restaurant+" 4"            
						},
						{
							"text": "6",
							"callback_data": "nosho 1 "+sellerId + " "+restaurant+" 6"             
						},
						{
							"text": "8",
							"callback_data": "nosho 1 "+sellerId + " "+restaurant+" 8"            
						}
						]
					]
				}
